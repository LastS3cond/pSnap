...
self.onmessage = function(e) {
    ...
    try {
        ...
        else if (type === 'loop' || type === 'foreach') {
            try {
                // Parallel For Loop
                ...
                const workerCode = 
                    varDecls + 
                    (isLoop 
                        ? 'for (let ' + arg + ' = ' + data.start + '; ' + arg + ' <= ' + data.end + '; ' + arg + '++) {' 
                        : 'for (let i = 0; i < data.length; i++) {' + arg + ' = data[i];'
                    ) +
                        msg.code +
                    '}' + 
                    'return ' + returnObj + ';';
...
function parallelMap(workers, jsArray, mapperBody, mapParams, reducerBody, reduceParams) {
    ...
    return Promise.all(
        ...
        return results.filter(r => r !== null && r !== undefined).flat();
    });
}
...
function prepVariables(firstPrivate, privateVars, lastPrivateVars, sharedVars, reductionVars, variables) {
    ...
    if (firstPrivate instanceof List) {
            ...
            return {[varName]: variables.getVar(varName)};
        });
    }
...
async function runWorkers(chunks, functionType, jsCode, paramNames, firstPrivateList, privateList, lastPrivateList, sabVars, reductionList, reductionCodes, initializations, variables) {
    ...
    return Promise.all(
        workers.map((worker, i) => {
            return new Promise((resolve, reject) => {
                worker.onmessage = msg => {
                    if (msg.data.type === 'done') resolve(msg.data.results);
        ...
        if (lastPrivateList.length > 0) {
            ...
            lastPrivateList.forEach(name => {
                if (lastResult && lastResult[name] !== undefined) {
                    variables.setVar(name, lastResult[name]);
                }
            });
        }
            ...
            reductionList.forEach((varName, index) => {
                ...
                if (code) {
                    ...
                    if (validValues.length > 0) {
                        ...
                        variables.setVar(varName, finalValue);
                    }
                }
            });
        }
